...
# NEW FUNCTION: Detect and remove header rows mixed in data
def detect_and_remove_header_rows(df):
    ...
    # Remove rows where Cancer_Type equals the column name itself or similar
    header_mask = df['Cancer_Type'].str.contains('Cancer_Type|cancer_type|Cancer Type', case=False, na=False)
    
    if header_mask.any():
        print(f"Found {header_mask.sum()} header rows mixed in data:")
        print(df[header_mask]['Cancer_Type'].tolist())
        df = df[~header_mask].copy()
        print(f"Removed header rows. New dataset size: {len(df)}")
    
    # Remove rows where numerical columns contain column names or non-numeric text
    for col in ['Age', 'WBC_Count', 'Platelet_Count']:
        if col in df.columns:
            # Check for column names in data
            text_mask = df[col].astype(str).str.contains(col.replace('_', '|'), case=False, na=False)
            if text_mask.any():
...
# NEW FUNCTION: Validate cancer types
...
# NEW FUNCTION: Safe train-test split
...
# APPLY HEADER DETECTION BEFORE ANY PROCESSING
...
# APPLY CANCER TYPE VALIDATION
df = validate_cancer_types(df)
...
def validate_and_convert_data_types(df):
    """Validate and convert data types for numerical columns"""
    print("=== Data Type Validation and Conversion ===")
    
    # Define expected numerical columns
    numerical_cols = ['Age', 'WBC_Count', 'Platelet_Count']
    ...
    for col in numerical_cols:
        if col in df.columns:
            ...
            print(f"  After conversion dtype: {df[col].dtype}")
            print(f"  NaN values created: {df[col].isnull().sum()}")
    
    return df

def inspect_data_quality(df):
    """Inspect data quality and identify issues"""
...
def plot_data_distribution(df):
    ...
    plt.tight_layout()
    plt.savefig('public/svm_results/data_distribution_4features.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def detect_and_remove_outliers(df, contamination=0.05):
    ...
    plt.tight_layout()
    plt.savefig('public/svm_results/outlier_detection_4features.png', dpi=300, bbox_inches='tight')
    plt.show()
...
# UPDATED: Use safe train-test split
...
# OPTIMIZED: Much smaller parameter grid for faster training
print("🔄 Performing FAST hyperparameter tuning with optimized GridSearchCV...")

# Step 1: Quick initial screening with fewer parameters
initial_param_grid = {
    'C': [1, 10, 100],
    'kernel': ['rbf', 'linear'],
...
print(f"Initial search space: {len(initial_param_grid['C']) * len(initial_param_grid['kernel']) * len(initial_param_grid['gamma']) * len(initial_param_grid['class_weight'])} combinations")

# Create SVM classifier
svm_model = SVC(random_state=42, probability=True)

# Use StratifiedKFold for better cross-validation (reduced folds for speed)
cv_strategy = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)

# Initial quick search
print("Step 1: Quick initial parameter search...")
...
# Fit the initial search
initial_search.fit(X_train_scaled, y_train)

print(f"✅ Initial best parameters: {initial_search.best_params_}")
print(f"✅ Initial best CV score: {initial_search.best_score_:.4f}")

# Step 2: Refined search around best parameters
print("\nStep 2: Refined search around best parameters...")

best_kernel = initial_search.best_params_['kernel']
best_C = initial_search.best_params_['C']
best_gamma = initial_search.best_params_['gamma']
best_class_weight = initial_search.best_params_['class_weight']

# Refined parameter grid based on initial results
if best_kernel == 'rbf':
    refined_param_grid = {
        'C': [best_C * 0.1, best_C, best_C * 10],
        'kernel': ['rbf'],
        'gamma': [best_gamma * 0.1, best_gamma, best_gamma * 10] if best_gamma != 'scale' else ['scale', 0.001, 0.01],
...
print(f"Refined search space: {np.prod([len(v) for v in refined_param_grid.values()])} combinations")
...
# Get the best model
best_svm_model = refined_search.best_estimator_
...
summary = {
    'algorithm': 'SVM',
    'dataset': '4 features only (Age, WBC_Count, Platelet_Count, Cancer_Type)',
    'original_size': len(df),
    ...
    'feature_names': X.columns.tolist(),
    'visualizations': {
        'feature_importance': 'public/svm_results/svm_feature_importance_4features.png',
        'confusion_matrix': 'public/svm_results/svm_confusion_matrix_4features.png',
        'cv_scores': 'public/svm_results/svm_cv_scores_4features.png',
        'decision_boundary': 'public/svm_results/svm_decision_boundary_4features.png'
    }
}

summary_file = 'public/svm_results/svm_summary_4features.json'
with open(summary_file, 'w') as f:
    json.dump(summary, f, indent=4)
print(f"✅ Summary saved to: {summary_file}")
print(f"\n=== SVM Blood Cancer Analysis Completed Successfully! ===")